...
Meta teammate span filter: teammate callback-injected `tool_use` and
`tool_result` rows can now be selectively hidden via the `t` key.
Toggling hides/shows only new events; existing events remain visible
(DataTable row API limitation). Footer shows `[meta:hidden]` when active.
238 tests passing.

### Added

- **Meta span filter toggle via `t` key** — hides/shows `tool_use` and
  `tool_result` events within teammate callback spans (generated by
  OMC's `meta user message` rows). Useful for decluttering the Timeline
  when reviewing a session with heavy callback instrumentation.
  - `hide_meta` state stored per Timeline panel
  - Toggle is event-prospective: new events respect the filter setting,
    but existing displayed rows are not retroactively hidden (DataTable
    row hide API limitation; workaround is clear + replay)
  - Footer shows `[meta:hidden]` when enabled
  - Persists until session switch (when timeline is cleared)

### Changed

- **Timeline meta span tracking** — added `_in_meta_span` state machine
  to track entry/exit of teammate callback meta spans via `is_meta`
  detection on `user_message` events. Never-raise parsing ensures
  malformed detection doesn't crash (AC-10, fallback to `is_meta=False`).
- **Version bumped to 0.8.0** (from 0.7.0) to accommodate token usage
  section in Turn Summary modal (v0.8.0 now shows three data classes:
  token totals, main-session vs. agent/skill attribution, and graceful
  rendering for legacy sessions without usage data).

### Tests

- Full suite: **238 passing**. New tests cover meta span detection,
  toggle action, footer rendering, and never-raise parsing on
  malformed `is_meta` hints.
