...
class CallGraph:
    ...
    def _handle_subagent_tool_use(self, ev: HarnessEvent) -> bool:
        """Route an internal tool_use event from a subagent file to its
        parent Agent node's tool_breakdown. Events whose subagent_uuid
        doesn't map to a known node are silently dropped — this can
        happen if the subagent file is seen before the main-session
        tool_result that links it. Returns True if the breakdown
        actually changed.
        """
